...
import com.waimaoshensou.google.search.ProxyProperty;
...
public class HttpclientUtil {
	...
	private static final int HTTP_CODE_302 = 302;
	private static final String IPV4_GOOGLE = "https://ipv4.google.com";
	...
	private static ResponseHandler<String> responseHandler = new ResponseHandler<String>() {
		...
		public String handleResponse(HttpResponse response) throws ClientProtocolException, IOException {
			...
			if (HTTP_CODE_302 == response.getStatusLine().getStatusCode()) {
				Header[] hh = response.getHeaders("Location");
				if (hh.length > 0 && hh[0].getValue().contains(IPV4_GOOGLE)) {
					System.out.println("更换代理IP");
				}
			}
	...
	public static String getByProxy(String url, Map<String, String> params, String charset) {
		if (url == null || StringUtils.isEmpty(url)) {
			return null;
		}
		List<NameValuePair> qparams = getParamsList(params);
	...
	private static DefaultHttpClient getDefaultHttpClientByProxy(final String charset) {
		DefaultHttpClient httpclient = new DefaultHttpClient();
		HttpHost proxy = null;
		Map<Object, Object> map = ProxyProperty.getProxy();
		if (map.size() == 0) {
	 ...
	 * @throws IOException
	 */
	public static String getDocument(String strUrl, String charset) throws IOException {
	...
	public static boolean checkProxy(String ip, String port) {
		...
		try {
			...
			httpclient.getParams().setParameter(CoreProtocolPNames.USER_AGENT,
					"Mozilla/5.0 (Windows; U; Windows NT 6.1; zh-CN; rv:1.9.2.13) Gecko/20101203 Firefox/3.6.13");// 我本机实际的User-Agent
			httpclient.getParams().setParameter(CoreProtocolPNames.USE_EXPECT_CONTINUE, Boolean.FALSE);
			httpclient.getParams().setParameter(CoreProtocolPNames.HTTP_CONTENT_CHARSET, CHARSET_UTF8);
			httpclient.setHttpRequestRetryHandler(requestRetryHandler);
			HttpParams params = httpclient.getParams();
			HttpConnectionParams.setConnectionTimeout(params, connectionTimeoutMillis);
			HttpConnectionParams.setSoTimeout(params, socketTimeoutMillis);
			HttpHost targetHost = new HttpHost("http://www.google.com.hk", 80, "http");
			HttpGet httpget = new HttpGet("/");
			HttpResponse response = httpclient.execute(targetHost, httpget);
			System.out.println(ip + ":" + port + "=" + response.getStatusLine());
